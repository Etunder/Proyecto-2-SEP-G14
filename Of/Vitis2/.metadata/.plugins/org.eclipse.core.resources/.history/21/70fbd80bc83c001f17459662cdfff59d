...
static int ReadFileFromSDWithIndex(const char* Filename, uint32_t* Buffer, UINT BufferSize, UINT StartIndex);
...
int main(void) {
    ...
    int file_count = 0;
    int file_index;
    UINT startIndex = BUFFER_SIZE * 500;
    ReadResult result;
	...
	for (int i = 0; i < BUFFER_SIZE; i++) {
	    xil_printf("Raw data[%d]: %u \r\n", i, result.buffer[i]);
	    Xil_Out32(XPAR_AXIFLOAT_1_S00_AXI_BASEADDR, result.buffer[i]);
	    usleep(100000);
	}
